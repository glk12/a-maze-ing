...
def validate_maze(
    ...
    if not directions and entry != exit_coord:
        raise MazeValidationError("Maze has no valid path between entry and exit.")
...
def solve_shortest_path(
    ...
    previous: dict[Coordinate, tuple[Coordinate, Direction] | None] = {entry: None}
...
def _validate_borders(maze: Maze) -> None:
    ...
    for x in range(maze.width):
        ...
        if not maze.cell_at(Coordinate(x, maze.height - 1)).has_wall(Direction.SOUTH):
    ...
    for y in range(maze.height):
        ...
        if not maze.cell_at(Coordinate(maze.width - 1, y)).has_wall(Direction.EAST):
